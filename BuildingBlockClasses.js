...
class ScheduleAssignmentPop extends Popup {
    constructor(parentTable) {
        super();
        this.taskDescriptionLine = document.createElement("div");
        this.loadCardDatapoints = this.loadCardDatapoints.bind(this);
        this.subChanges = this.subChanges.bind(this);
        //this.submitButton = new Button();
        //this.submitButton.setTitle("SUBMIT");

        this.parentTable = parentTable;
        this.subjectTable = parentTable;

        this.titleDescriptionLineTopics = [
            //"parentProject",
            "masterTraveler",
            "opCode",
            "desc",
        ...
        this.datepick = new DatePicker("2024-12-01", "2025-12-31", "2024-12-09");
        this.mainView.contentBucket.appendChild(this.datepick.el);
        this.mainView.contentBucket.appendChild(this.taskDescriptionLine);
    }

    loadCardDatapoints() {
        for (let i = 0; i < this.titleDescriptionLineTopics.length; i++) {
            let newDatapointFromLive = new CardDatapointFromLive(
                this.titleDescriptionLineTopics[i],
                this.subjectRow,
                this.titleDescriptionLineTopics[i]
            );
            this.taskDescriptionLine.appendChild(newDatapointFromLive.el);
        }




        this.privelegeTabTable = new MultiSelectFromTabFiltered("User Privileges", allUsers, "id", allUserPrivileges, "privilegedUser");

        this.privelegeTabTable.setInclusions([
        ...
        //privelegeTabTable.setFeatures(["rowDeleteButton"]);



        this.privelegeTabTable.turnDroppingOn();
        this.privelegeTabTable.setTableBodyFlexDirection("column");




        this.mainView.contentBucket.appendChild(this.privelegeTabTable.el);
    ...
    subChanges() {
        ...
        for (let i = 0; i < this.privelegeTabTable.selectedItems.length; i++) {
            let newID = this.subjectRow.id.value.concat(
                this.privelegeTabTable.selectedItems[i],
                this.datepick.value
            );
            let dateTime = new Date(this.datepick.value.concat("T11:51:00"));
            //new Date('2011-04-11T11:51:00');
            //let formattedDateTime = this.datepick.value.format('YYYY-MM-DD HH:mm:ss');
            let recordJSON = {
                id: newID,
                oiqoc_task_id: this.subjectRow.id.value,
                wadgn_assigned_operator: this.privelegeTabTable.selectedItems[i],
                zxxfx_date_assigned_for: dateTime,
                fqctj_status: "Open",
                smnun_assignment_performed_by: liu.id,
                dxngn_time_of_assignment: date,
            };
            let newAssignment = new TaskAssignment();
            newAssignment.addVars(recordJSON);
            allAssignments.addRecord(recordJSON, newAssignment);
    ...
    addWeldJointTable() {
        let weldJoints = new WeldJointTable();
        weldJoints.addRequestCriteria(
            new RequestFilterCriteria(
                "dxfvg_parent_project",
                "equal",
                this.subjectRow.parentProject.value
            )
        );

        weldJoints.setTitle("Weld Joints");
        weldJoints.turnDroppingOn();
        weldJoints.setTableBodyFlexDirection("column");
        weldJoints.close();

        weldJoints.addRequestCriteria(
            new RequestFilterCriteria(
                "remgl_parent_master_traveler",
                "equal",
                this.subjectRow.masterTraveler.value
            )
        );

        weldJoints.setInclusions([
            { columnString: "jointType", staticBool: true },
            { columnString: "weldSize", staticBool: true },
            { columnString: "baseMaterialListString", staticBool: true },
            { columnString: "baseMaterialThicknessListString", staticBool: true },
            { columnString: "subjectRowList", staticBool: false },
        ]);

        weldJoints.generateRequestObject();

        let weldJointArray =
            this.subjectRow.masterTaskRow.weldJoints.value.split(",");

        weldJoints.filterItemsByColumnValueInArray("jointNumber", weldJointArray);

        //let waiterBob = new WhenFlagTrueCallFunction(
        //    weldJoints,
        //    "queryFinishedFlag",
        //    weldJoints,
        //    "setStaticContainerFlexDirection",
        //    "row"
        //);
        //let waiterBob2 = new WhenFlagTrueCallFunction(
        //    weldJoints,
        //    "queryFinishedFlag",
        //    weldJoints,
        //    "setOverallFlexDirection",
        //    "column"
        //);

        //let waiterBob3 = new WhenFlagTrueCallFunctionMultiParams(
        //    weldJoints,
        //    "queryFinishedFlag",
        //    weldJoints,
        //    "addAggregationToSummary",
        //    ["UniqueValues", "jointNumber", "Unique Joint Numbers: "]
        //);

        this.el.append(weldJoints.el);

        //let newButton = new TableButton();
        //newButton.addFunctionWithParamViaLocation(
        //    this,
        //    "setSelectedUser",
        //    "self",
        //    "parentTable"
        ...
        //newButton.setTitle("Add To List Of Acceptable Operators");

        //this.privelegeTabTable.addPremadeButtonToAllTables(newButton);
    }
}
...
class TaskScheduleManager extends Popup {
    constructor() {
        super();
        this.taskDescriptionLine = document.createElement("div");

        this.addWeldJointTable = this.addWeldJointTable.bind(this);
        this.loadCardDatapoints = this.loadCardDatapoints.bind(this);
        this.subChanges = this.subChanges.bind(this);

        //this.taskDescriptionLine.className = "";

        this.weldJointTableContainer = document.createElement("div");

        this.titleDescriptionLineTopics = ["masterTraveler", "opCode", "desc"];

        this.userInputFieldsBucket = document.createElement("div");

        this.mainView.precontentBucket.appendChild(this.userInputFieldsBucket);

        this.applyPriorityLine = document.createElement("div");

        this.rangeSlider = new RangeSlider(0, 100, 50, 1);
        this.rangeSlider.setLabel("Task Priority:");
        this.rangeSlider.setValue(52);
        this.applyPriorityLine.appendChild(this.rangeSlider.el);

        this.userInputFieldsBucket.appendChild(this.applyPriorityLine);
        this.mainView.contentBucket.appendChild(this.weldJointTableContainer);
        this.userSelectionContainer = document.createElement("div");
        this.mainView.contentBucket.appendChild(this.userSelectionContainer);

        //newMultiWindowTrial.addFilterParameter({filterColumnName:"process",filterColumnValue:"GTAW"})
    }
    ...
    loadCardDatapoints() {
        ...
        //if (this.subjectRow.qualGroup.value == "WLD") {
        //    this.addWeldJointTable();
        //}

        this.mainView.precontentBucket.appendChild(this.taskDescriptionLine);

        this.rangeSlider.setValue(this.subjectRow.priority.value);

        //this.selectedOperatorsCardDatapoint = new CardDatapointFromLive(
        //    "Selected Operators",
        //    this.subjectRow.selectedOperators.value
        //);
        //this.userInputFieldsBucket.appendChild(
        //    this.selectedOperatorsCardDatapoint.el
        //);

        this.taskAssignments = new TableWindowFromLiveTable();
        this.taskAssignments.addPostRequestFilter(
            "taskID",
            this.subjectRow.id.value,
            "EQUAL"
        );
        this.taskAssignments.setFeatures(["rowDeleteButton", "editRecordButton"]);

        this.taskAssignments.setInclusions([
            { columnString: "taskID", staticBool: true, displayName: "Task ID" },
            {
                columnString: "assignedOperator",
                staticBool: true,
                displayName: "Assigned Operator",
            },
            {
                columnString: "dateAssignedFor",
                staticBool: true,
                displayName: "Assignment Scheduled For Date",
            },
            {
                columnString: "status",
                staticBool: true,
            ...
            {
                columnString: "assignmentPerformedBy",
                staticBool: true,
                displayName: "Assignment Performed By",
            },
        ]);
        this.taskAssignments.setParentLiveTable(allAssignments);
        this.taskAssignments.setTitle("Assignments For This Task");
        //unassignedTasksDisplayTable.addRowDetailButtons("Manage Task", "TaskScheduleManager");

        //this.allAssignments = new TaskAssignmentTable();

        //this.allAssignments.setTitle("Assignments For Selected Task");
        //this.allAssignments.omitSummaryDrop();

        //this.allAssignments.addRequestCriteria(
        //    new RequestFilterCriteria("oiqoc_task_id", "equal", this.subjectRow.id.value)
        //);

        //this.allAssignments.setInclusions([
        //    { columnString: "taskID", staticBool: true },
        //    { columnString: "assignedOperator", staticBool: true },
        //    { columnString: "dateAssignedFor", staticBool: true },
        //    { columnString: "assignmentPerformedBy", staticBool: true },
        //    { columnString: "timeOfAssignment", staticBool: true },
        //]);

        //this.allAssignments.generateRequestObject();

        this.mainView.precontentBucket.appendChild(this.taskAssignments.el);


        this.sap = new SingleMultiplcityRecordGenForm();
        this.sap.setRecordType("TaskAssignment");
        this.sap.setParentRecord(this.subjectRow);
        this.sap.setParentLiveTable(allAssignments);
        this.sap.setParentTableWindow(this.taskAssignments);
        this.sap.buildForm();

        this.assignPopupButton = new ButtonToPremadePopup(this.sap);

        this.assignPopupButton.setTitle("Add Assignment");

        this.mainView.precontentBucket.appendChild(this.assignPopupButton.el);

        this.open();
    }

    subChanges() {
        this.subjectRow.priority.updateValue(this.rangeSlider.getValue());
        //this.subjectRow.selectedOperators.updateValue(
        //    this.selectedOperatorsCardDatapoint.value
        //);
        this.close();
    }
    ...
    addWeldJointTable() {
        ...
        this.weldJointTableContainer.appendChild(weldJoints.el);
    }

    setSelectedUser(selectedUserParentTable) {
        let value = selectedUserParentTable.subjectID;
        this.selectedOperatorsCardDatapoint.forcePush(value);
        this.selectedOperatorsCardDatapoint.forceRebuild(value);
    }
}
